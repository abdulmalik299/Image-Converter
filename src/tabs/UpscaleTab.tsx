...
export function UpscaleTab({ settings, setSettings }: { settings: CommonRasterSettings; setSettings: (up: (p: CommonRasterSettings) => CommonRasterSettings) => void }) {
  ...
  const computeTargetSize = async (file: File) => {
    ...
    const width = Math.round(baseScaleW * targetScale);
    const height = Math.round(baseScaleH * targetScale);
    bmp.close?.();
    return { width, height };
  };
  ...
  }, [firstItem?.id, scale, settings.quality, settings.sharpenAmount, settings.aiEnhance, settings.aiContrast, settings.aiSaturation, settings.aiExposure, settings.aiDenoise, settings.autoColor, settings.smoothing, settings.smoothingQuality, settings.out, autoOut, detailRecovery, resampleStrength, ultraHdLongEdge]);
          ...
          <Card title="Upscale settings" subtitle="Power level 100 controls for difficult low-quality images.">
            <div className="space-y-4">
              ...
              <Field label="Upscale factor" hint={`${scale}x`}>
                <Slider min={2} max={12} value={scale} onChange={(e) => setScale(Number(e.target.value))} />
              </Field>
              ...
              <Field label="Quality" hint={`${settings.quality}%`}>
                <Slider min={80} max={100} value={settings.quality} onChange={(e) => setSettings((p) => ({ ...p, quality: Number(e.target.value) }))} />
              </Field>
