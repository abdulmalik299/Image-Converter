...
self.onmessage = (event: MessageEvent<RenderRequest>) => {
  ...
  const temp = new OffscreenCanvas(workW, workH);
  ...
  tctx.drawImage(request.sourceBitmap, cropX, cropY, cropW, cropH, -workW / 2, -workH / 2, workW, workH); request.sourceBitmap.close();
  const ctx = temp.getContext("2d", { willReadFrequently: true }); if (!ctx) return;
  ...
  const outCanvas = new OffscreenCanvas(outW, outH); const outCtx = outCanvas.getContext("2d"); if (!outCtx) return;
